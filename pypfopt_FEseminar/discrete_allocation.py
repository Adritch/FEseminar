...
class DiscreteAllocation:
    ...
    def _allocation_rmse_error(self, verbose=True):
        ...
        for ticker, weight in self.weights:
            ...
            if verbose:
                print(
                    "{}: allocated {:.5f}, desired {:.5f}".format(
    ...
    def greedy_portfolio(self, reinvest=False, verbose=False):
        ...
        for ticker, weight in self.weights:
            ...
            n_shares = np.floor(weight * self.total_portfolio_value / price)
            cost = n_shares * price
        ...
        # Get RMSE
        rmse = self._allocation_rmse_error(verbose=False)
            
        return self.allocation, available_funds, rmse
    ...
    def lp_portfolio(self, reinvest=False, verbose=False, solver="GLPK_MI"):
        ...
        opt.solve(solver=solver) # Set verbose=True if u want to see solver output
        ...
        return self.allocation, r.value, rmse
